...
3. 문체 및 작성 규칙:
   ...
   - 순수 HTML 내용만 작성할 것 (\`\`\`html 코드 블록 마크다운이나 <html>, <body> 태그는 일체 쓰지 말 것).`;
    ...
    for (const model of candidateModels) {
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${this.apiKey}`, {
          ...
          body: JSON.stringify({
            systemInstruction: {
              parts: [{ text: "너는 대한민국 최고 권위의 탐사 저널리스트이자 수석 논설위원이야. 주어진 이슈를 바탕으로 독자에게 실질적인 인사이트를 주는 1,500자 이상의 고품질 장문 심층 분석 기사를 완성도 높은 HTML 태그로 작성한다. 첫 줄에 'TITLE: 제목'을 적고 본문 HTML만 출력한다." }]
            },
        ...
        if (rawOutput && rawOutput.length >= 250) {
    ...
    // 비상 상황 시에도 각 항목(첫째, 둘째, 셋째)이 누락 없이 완벽하게 갖춰진 장문 리포트 제공
    console.warn("⚠️ AI 응답 한계로 트렌드 팩트 기반 정밀 구조화 리포트로 대체 구성합니다.");
    const fallbackTitle = `[심층 리포트] ${trend.title} 사태 집중 분석 - 현안 쟁점과 향후 파장`;
    const newsInfo = trend.newsTitle ? `"${trend.newsTitle}"` : `${trend.title} 관련 주요 현안`;
    const snippetInfo = trend.snippet || `${trend.title}에 관한 사회적 관심과 언론 보도가 급증하고 있는 상황입니다.`;

    const fallbackHtml = `
<h2>1. 사태의 발단과 전개 과정: 구체적 팩트와 타임라인</h2>
<p>최근 실시간으로 가장 뜨겁게 회자되고 있는 <strong>${trend.title}</strong> 이슈가 대중과 각계 전문가들 사이에서 커다란 반향을 일으키고 있습니다.</p>
<p>주요 언론 보도에 따르면 ${newsInfo} 소식이 신속하게 전해지면서 이에 대한 사실관계 확인과 후속 취재가 잇따르고 있습니다. 특히 현장 상황과 관련하여 ${snippetInfo}</p>
<p>이번 사안은 표면적으로 드러난 단순한 일회성 해프닝을 넘어, 현장의 복합적인 정황과 제도적 한계가 맞물리면서 사태의 파장이 한층 증폭되는 양상입니다.</p>

<h2>2. 수면 위로 드러난 핵심 쟁점과 찬반 여론</h2>
<p>이번 <strong>${trend.title}</strong> 사안을 둘러싸고 온라인 커뮤니티와 여론의 시선은 팽팽하게 맞서고 있습니다. 주요 논쟁 포인트는 다음과 같이 요약됩니다.</p>
<ul>
...
<h2>3. 구조적 원인과 배경: 겉으로 드러나지 않은 숨은 맥락</h2>
<p>이번 <strong>${trend.title}</strong> 사안의 이면에는 오랫동안 누적되어 온 복합적인 구조적 요인들이 자리잡고 있습니다.</p>
<p><strong>첫째, 제도와 현실의 괴리입니다.</strong> 급변하는 대내외 환경과 대중의 눈높이에 비해 기존 규정과 관리 가이드라인이 현실의 복잡다단한 상황을 충분히 반영하지 못하고 있다는 점이 주요 원인으로 꼽힙니다.</p>
<p><strong>둘째, 예방적 관리 시스템의 부재입니다.</strong> 유사한 위험 징후나 갈등 요소가 사전에 충분히 감지되었음에도 불구하고, 이를 선제적으로 조율하고 차단할 수 있는 컨트롤 타워와 대응 프로토콜이 제대로 작동하지 못했습니다.</p>
<p><strong>셋째, 소통과 정보 공개의 비대칭성입니다.</strong> 사건 발생 초기 투명한 정보 공유와 적극적인 해명이 부족했던 탓에 불필요한 오해와 불신이 증폭되었으며, 이는 사태를 단순한 논란에서 구조적 불신으로 키우는 결정적 요인으로 작용했습니다.</p>

<h2>4. 전문가 진단 및 향후 시장·사회적 파급 효과</h2>
<p>전문가들은 이번 사건이 단순한 화제몰이에 그치지 않고 관련 업계 및 사회 전반에 걸쳐 상당한 제도적 변화를 몰고 올 것으로 전망하고 있습니다.</p>
<p>단기적으로는 관계 당국의 규제 점검 및 운영 방침의 대대적인 정비가 불가피할 것으로 보이며, 중장기적으로는 투명성과 공정성을 확보하기 위한 새로운 표준이 수립되는 계기가 될 가능성이 높습니다. 또한 소비자들과 대중의 신뢰 회복 여부가 향후 관련 분야의 성패를 가를 핵심 잣대가 될 것입니다.</p>

<h2>5. 시사점 및 총평: 앞으로 주목해야 할 관전 포인트</h2>
<p>결국 이번 <strong>${trend.title}</strong> 논란은 우리 사회가 당면한 과제를 얼마나 성숙하고 책임감 있게 풀어나갈 수 있는지를 가늠하는 중요한 시험대입니다.</p>
<p>앞으로 공식적인 후속 발표와 추가 검증 결과에 따라 사태의 향방이 결정될 것이며, 단편적인 공방을 넘어 실효성 있는 근본 대책이 마련되는지 지속적인 관심과 모니터링이 필요한 시점입니다.</p>
`;
